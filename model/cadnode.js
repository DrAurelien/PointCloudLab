var CADNode = (function () {
    ...
    CADNode.prototype.GetActions = function (dataHandler, onDone) {
        ...
        var result = [];
        if (this.deletable) {
            result.push(new Action('Remove', function () { self.owner.Remove(self); return onDone(null); }));
        }
        if (this.visible) {
            result.push(new Action('Hide', function () { self.visible = false; return onDone(null); }));
        }
        else {
            result.push(new Action('Show', function () { self.visible = true; return onDone(null); }));
        }
        return result;
    };
